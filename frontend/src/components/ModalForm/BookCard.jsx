...
// Simple responsive card grid for an array of book objects
const BookCard = ({ data, onEdit, onDelete, onBorrow, role, isBorrowed, onReturn }) => {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
      {data.map((book, idx) => (
        <div key={book.id || idx} className="bg-white rounded shadow p-4 flex flex-col relative">
          ...
          <p className="text-xs text-gray-700 mb-1"><span className="font-semibold">Author:</span> {book.author || 'Unknown'}</p>
            {book.isbn ? (
          ...
          {/* Student: show Borrow or Return button based on isBorrowed */}
          {role === 'student' && (
            <button
              className={`mt-3 px-4 py-2 rounded transition ${
                isBorrowed ? 'bg-red-600 hover:bg-red-700 text-white' : 'bg-blue-600 hover:bg-blue-700 text-white'
              }`}
              onClick={() => isBorrowed ? handleReturn(book) : handleBorrow(book)}
            >
